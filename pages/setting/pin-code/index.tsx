...
import { PincodeColumns } from '@/components/setting/pin-code/pin-code';
import { IPINCode } from '@/types/interfaces/pinCode';
import { useGetAllPINCodelistQuery } from '@/features/pinCode/pinCodeAPI';

export default function PINCodeIndex({ className }: React.HTMLAttributes<HTMLDivElement>) {
    const router = useRouter();
    const { pageIndex, pageSize, onPaginationChange, paginator } = usePagination();
    const { sorting, onSortingChange, field, order } = useSorting();
    const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([]);
    const debouncedColumnFilters: ColumnFiltersState = useDebounce(columnFilters, 1000);
    const { data, isLoading } = useGetAllPINCodelistQuery(
        {
    ...
    // console.log('🚀 ~ PINCodeIndex ~ data:', data);
    ...
    const renderEmpty = () => {
        return <div>No Pincode Available!!!</div>;
    };
            ...
            <div className="grid grid-cols-12 gap-3">
                <div className="col-span-12 sm:col-span-4">
                    <Breadcrumb>
                        <BreadcrumbList>
                            ...
                            <BreadcrumbItem>
                                <BreadcrumbPage>PIN Code</BreadcrumbPage>
                            </BreadcrumbItem>
                        </BreadcrumbList>
                    </Breadcrumb>
                    <h2 className="text-2xl tracking-tight mb-1 mt-1 font-poppins font-medium">All PIN Code</h2>
                </div>
                <div className="col-span-12 sm:col-span-8">
                    <div className="flex justify-end flex-col sm:flex-row gap-4">
                        ...
                        <Button className="py-1 px-3" onClick={() => router.push('/setting/pin-code/add-edit')}>
                            <BadgePlus className="mr-2 w-4 h-4" /> New
                        </Button>
                        {/* <Button className="py-1 px-3" onClick={() => router.push('/setting/pinCode/add-edit')}>
                             <BadgePlus className="mr-2 w-4 h-4" /> New Zone
                         </Button> */}
                    </div>
                </div>
            </div>
