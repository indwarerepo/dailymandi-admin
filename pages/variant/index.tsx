...
import { VariantColumns } from '@/components/variant/variant';
import { useGetAllVariantlistQuery } from '@/features/variant/variantAPI';

export default function VariantIndex({ className }: React.HTMLAttributes<HTMLDivElement>) {
    const router = useRouter();
    const { pageIndex, pageSize, onPaginationChange, paginator } = usePagination();
    const { sorting, onSortingChange, field, order } = useSorting();
    const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([]);
    const debouncedColumnFilters: ColumnFiltersState = useDebounce(columnFilters, 1000);
    const { data, isLoading } = useGetAllVariantlistQuery(
        {
            ...
            <div className="grid grid-cols-12 gap-3">
                ...
                <div className="col-span-12 sm:col-span-8">
                    <div className="flex justify-end flex-col sm:flex-row gap-4">
                        ...
                        <Button className="py-1 px-3" onClick={() => router.push('/variant/add-edit')}>
