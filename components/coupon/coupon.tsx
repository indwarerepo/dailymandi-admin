...
import { useDeleteCouponMutation, useEditActiveStatusMutation } from '@/features/coupon/couponAPI';
    ...
    async function updateStatus(id: string) {
        try {
            ...
            if (res.statusCode === 201) {
                toast.success(`🎉 Coupon Status Successfully Updated.`);
            } else {
...
export const CouponColumns: ColumnDef<ICoupon>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => <div className="text-sm justify-start">{row.getValue('couponCode')}</div>,
    },
    {
        ...
        cell: ({ row }) => <div className="text-sm text-center">{row.getValue('minOrderAmount')}</div>,
    },

    {
        ...
        cell: ({ row }) => <div className="text-sm text-center">{row.getValue('offerPercentage')}</div>,
    },
    {
        accessorKey: 'couponValidity',
        header: ({ column }) => {
            return (
                <Button
                    variant="ghost"
        ...
        cell: ({ row }) => <div className="text-sm text-center">{row.getValue('couponValidity')} Days</div>,
    },
    {
        ...
        cell: ({ row }) => <div className="text-sm text-center">{row.getValue('useLimit')}</div>,
    },
    {
        ...
        cell: ({ row }) => (
            <div className="text-sm text-center">{format(new Date(row?.original?.startDate), 'dd-MM-yyyy')}</div>
        ),
    },
    {
        ...
        cell: ({ row }) => (
            <div className="text-sm text-center">{format(new Date(row?.original?.expiredDate), 'dd-MM-yyyy')}</div>
        ),
    },
    {
