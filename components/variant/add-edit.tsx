...
import { ChevronLeft, CornerUpLeft, Plus, Save } from 'lucide-react';
import {
    Breadcrumb,
    BreadcrumbItem,
    BreadcrumbLink,
...
import { variantSchema } from '@/types/schemas';
import { Textarea } from '../ui/textarea';
import { IVariant } from '@/types/interfaces/variant';
import { useAddVariantMutation, useEditVariantMutation } from '@/features/variant/variantAPI';

type props = {
    selectedVariants: IVariant;
    setSelectedVariants: Dispatch<SetStateAction<IVariant>>;
    thisId: string;
    isUpdate: boolean;
};

const AddEditComponent = ({ selectedVariants, setSelectedVariants, thisId, isUpdate }: props) => {
    const router = useRouter();
    const [isLoading, setIsLoading] = useState<boolean>(false);
    const [addVariants] = useAddVariantMutation();
    const [editVariants] = useEditVariantMutation();
    ...
    const { values, errors, touched, handleBlur, handleChange, handleSubmit } = useFormik({
        ...
        onSubmit: async (values, action) => {
            ...
            try {
                let payload: any = {
                    variantName: values.variantName as string,
                    description: values.description as string,
                };
                if (isUpdate) {
                    payload.id = thisId;
                    let res = await editVariants(payload).unwrap();
                ...
                <div className="col-span-12 sm:col-span-4">
                    <Breadcrumb>
                        <BreadcrumbList>
                            ...
                            <BreadcrumbItem>
                                <BreadcrumbPage>{isUpdate ? `Edit` : `Add`} Variant</BreadcrumbPage>
                            </BreadcrumbItem>
                        </BreadcrumbList>
                    </Breadcrumb>
                    <h2 className="text-2xl tracking-tight mb-1 mt-1 font-poppins font-medium">
                        {' '}
                        {isUpdate ? `Edit` : `Add`} Variant
                    </h2>
                </div>
            </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 w-full items-center gap-7 mb-5">
                            <div className="flex flex-col space-y-1.5">
                                ...
                                {touched.variantName && errors.variantName ? (
                                    <small id="variantName-help" className="text-rose-600">
                                        {errors.variantName}
                                    </small>
                                ) : null}
                            </div>
