    ...
    async function updateStatus(id: string) {
        try {
            ...
            if (res.statusCode === 201) {
                toast.success(`🎉 Sub Category Status Successfully Updated.`);
            } else {
...
export const SubCategoryColumns: ColumnDef<ProductSubCategory>[] = [
    {
        accessorKey: 'name',
        header: ({ column }) => {
            return (
    ...
    },
    {
        ...
        cell: ({ row }) => <div className="text-sm">{row?.original?.product_category?.name}</div>,
    },
                            ...
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <Pencil
                                        ...
                                        onClick={() =>
                                            router.push({
                                                pathname: `/product-subcategory/add-edit`,
                                                query: {
                                                    id: row.getValue('id'),
                                                },
                                            })
...
const DeleteAlert = ({ id }: { id: string }) => {
    const [deleteProduct] = useDeleteProductSubCategoryMutation();
