...
import { useDeleteZoneMutation, useEditActiveStatusMutation } from '@/features/zone/zoneAPI';
    ...
    async function updateStatus(id: string) {
        try {
            ...
            if (res.statusCode === 201) {
                toast.success(`🎉 Zone Status Successfully Updated.`);
            } else {
...
export const ZoneColumns: ColumnDef<IZone>[] = [
    {
        ...
        cell: ({ row }) => <div className="text-sm justify-start">{row.getValue('zoneName')}</div>,
    },
    {
        accessorKey: 'area',
        header: ({ column }) => {
...

    {
        ...
        cell: ({ row }) => <div className="text-sm text-center">{row.getValue('deliveryCharge')}</div>,
    },
    {
                            ...
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <Pencil
                                        ...
                                        onClick={() =>
                                            router.push({
                                                pathname: `/setting/zone/add-edit`,
                                                query: {
                                                    id: row.getValue('id'),
                                                },
                                            })
